...
- [ ] T025 [P] [US1] Test: `ServicioPacientes.Crear` avisa (no bloquea) de duplicado por DNI activo (CA-003)
...
- [ ] T030 [US1] Implementar `IServicioPacientes` y `ServicioPacientes` en `src/Spd.Aplicacion/ServicioPacientes.cs` (Crear, Actualizar, CambiarEstado, Buscar, ValidarDni, ValidarCip, AutocompletarCip; depende de T006, T008, T009, T010, T012, T015); cada escritura registra en auditoría (Art. VII.6)
- [ ] T031 [US1] Implementar `FichaPacienteViewModel` en `src/Spd.Presentacion/ViewModels/FichaPacienteViewModel.cs` (pestaña Datos; cabecera con num_ficha/nombre/edad/estado/alertas, FR-008)
- [ ] T032 [US1] Implementar `FichaPacienteView`/`FichaPacienteWindow` en `src/Spd.Presentacion/Views/Pacientes/`
- [ ] T033 [US1] Implementar `BuscadorPacientesViewModel` + vista (listado, filtro por estado, FR-011) en `src/Spd.Presentacion/ViewModels/BuscadorPacientesViewModel.cs` + `src/Spd.Presentacion/Views/Pacientes/BuscadorPacientesView.axaml`
- [ ] T034 [US1] Añadir botón "Pacientes" a `MainWindow`/`MainViewModel` (visible para Elaborador y Administrador, FR-040)
...
- [ ] T035 [P] [US2] Test: `Buscar` de médicos no busca con menos de 2 caracteres y encuentra por fragmento normalizado (CA-004), en `tests/Spd.Aplicacion.Tests/ServicioMedicosTests.cs`
- [ ] T036 [P] [US2] Test: `Crear` médico avisa (no bloquea) si existe otro activo con mismos apellidos+nombre o mismo colegiado (FR-034)
- [ ] T037 [P] [US2] Test: `Actualizar` médico se refleja al releer un paciente que lo referencia, sin copia de datos (CA-006, Art. IV.1)
- [ ] T038 [P] [US2] Test: `DarDeBaja` médico lanza excepción con la lista de pacientes si es cabecera de alguno activo/evaluación (CA-007)
- [ ] T039 [P] [US2] Test: `Crear` y `DarDeBaja` de médico registran en auditoría (Art. VII.6)

### Implementation for User Story 2

- [ ] T040 [US2] Implementar `IServicioMedicos` y `ServicioMedicos` en `src/Spd.Aplicacion/ServicioMedicos.cs` (depende de T005, T008, T011, T014); cada escritura registra en auditoría
- [ ] T041 [US2] Implementar `SelectorMedicoViewModel` (autocompletado + "Nuevo médico…", FR-032/033) en `src/Spd.Presentacion/ViewModels/SelectorMedicoViewModel.cs` + vista reutilizable
- [ ] T042 [US2] Integrar `SelectorMedicoViewModel` en `FichaPacienteView` como campo de médico de cabecera (depende de T031/T032, T041)
- [ ] T043 [US2] Implementar `CatalogoMedicosViewModel` + vista (listado con búsqueda y nº de pacientes activos por médico, FR-037) en `src/Spd.Presentacion/ViewModels/CatalogoMedicosViewModel.cs` + vista
- [ ] T044 [US2] Añadir botón "Catálogo de médicos" a `MainWindow`/`MainViewModel`
...
- [ ] T045 [P] [US3] Test: contacto REPRESENTANTE_LEGAL/PERSONA_AUTORIZADA sin DNI no se guarda (CA-008), en `tests/Spd.Aplicacion.Tests/ServicioContactosTests.cs`
- [ ] T046 [P] [US3] Test: marcar `retira_medicacion` exige DNI (FR-021c) y desmarca cualquier otro contacto del mismo paciente que lo tuviera (FR-021b)
- [ ] T047 [P] [US3] Test: marcar `es_principal` desmarca cualquier otro contacto principal del mismo paciente (FR-021)
- [ ] T048 [P] [US3] Test: la baja de contacto es lógica, no se elimina y no aparece salvo "ver histórico" (FR-023, Art. III.1)
- [ ] T049 [P] [US3] Test: alta y baja de contacto registran en auditoría (Art. VII.6)

### Implementation for User Story 3

- [ ] T050 [US3] Implementar `IServicioContactos` y `ServicioContactos` en `src/Spd.Aplicacion/ServicioContactos.cs` (depende de T007, T013, T016); cada escritura registra en auditoría
- [ ] T051 [US3] Implementar pestaña "Contactos" (`ContactosPacienteViewModel` + vista) integrada en `FichaPacienteView` como segunda pestaña (FR-009; depende de T032)
...
- [ ] T052 [P] Ejecutar íntegramente [quickstart.md](./quickstart.md) y registrar el resultado en `PROGRESO.md`
- [ ] T053 Revisar que ningún método de `Spd.Dominio`/`Spd.Aplicacion` supere ~40 líneas ni ninguna clase ~300 (Art. XI.6)
- [ ] T054 [P] Actualizar `PROGRESO.md` marcando cada CA-001..CA-015 como validado, con el test que lo confirma
...
- **US2 (Fase 3)**: depende de Foundational; T042 (integrar el selector en la ficha) depende de
  T031/T032 de US1, el resto de US2 es independiente
- **US3 (Fase 4)**: depende de Foundational; T051 (pestaña Contactos) depende de T032 de US1
...
- Una vez completada Foundational: US1 puede avanzar en paralelo con la parte de US2/US3 que no
  depende de la ficha de paciente (T035-T040, T045-T050)
...
2. Fase 2 (US1) — **parar y validar** con CA-001/002/003/009/010/011 antes de seguir
